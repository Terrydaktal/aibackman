...
function stripGenUiArtifacts(value) {
...
function stripWritingDirectives(value) {
  return String(value || '')
    .replace(/(^|\n)[ \t]*:::writing(?:\{[^\n]*\})?[ \t]*/gi, '$1')
    .replace(/:::writing(?:\{[^\n]*\})?[ \t]*/gi, '')
    .replace(/(^|\n)[ \t]*:::[ \t]*(?=\n|$)/g, '$1')
...
function formatChatGptContent(value) {
  return normalizeFileCitationMarkers(stripWritingDirectives(stripGenUiArtifacts(value)))
    .replace(/\r\n?/g, '\n')
    .replace(/[ \t]+\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
...
module.exports = {
  formatChatGptContent,
  stripGenUiArtifacts,
  stripWritingDirectives,
  normalizeFileCitationMarkers,
};
